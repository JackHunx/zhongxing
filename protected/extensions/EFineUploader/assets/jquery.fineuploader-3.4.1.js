...
qq.FineUploaderBasic = function(o) {
    ...
    this._options = {
        ...
        messages: {
            typeError: "{file} 文件格式不被允许上传,请上传下列格式文件: {extensions}.",
            sizeError: "{file} 文件太大,上传的最大为 {sizeLimit}.",
            minSizeError: "{file} 文件太小,上传的文件赢不小于 {minSizeLimit}.",
            emptyError: "{file} 文件不存在,请重新选择文件.",
            noFilesError: "没有文件被上传.",
            tooManyItemsError: "太多文件 :({netItems}) 被上传. 文件最多限制为 {itemLimit}.",
            retryFailTooManyItems: "超过重试次数 - 你已经达到重试次数.",
            onLeave: "文件正在上传,如果离开将取消上传."
        },
        retry: {
            enableAuto: false,
            maxAutoAttempts: 3,
    ...
    _isBatchValid: function(validationDescriptors) {
        ...
        if (batchValid) {
            if (itemLimit === 0 || proposedNetFilesUploadedOrQueued <= itemLimit) {
                ...
                batchValid = false;
                errorMessage = this._options.messages.tooManyItemsError.replace(/\{netItems\}/g, proposedNetFilesUploadedOrQueued)
...
qq.UploadHandlerXhr = function(o, uploadCompleteCallback, logCallback) {
    ...
    function persistChunkData(id, chunkData) {
        var fileUuid = api.getUuid(id),
            ...
            cookieValue = fileUuid + cookieItemDelimiter + chunkData.part + cookieItemDelimiter + lastByteSent + cookieItemDelimiter + initialRequestOverhead + cookieItemDelimiter + estTotalRequestsSize,
    ...
    addCallbacks = function(transformedOpts) {
        ...
        $.each(uploaderInst._options.callbacks, function(prop, func) {
            ...
            callbacks[prop] = function() {
                ...
                if (jqueryHandlerResult === undefined && $.inArray(prop, uploaderInst.getPromissoryCallbackNames()) >= 0) {
                    return origFunc();
                }
